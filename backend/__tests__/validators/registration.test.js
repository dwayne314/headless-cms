...
describe("Validate Registration", () => {
  ...
  it("returns error object with a firstName error if the firstName is not in the request body", async () => {
    const req = mockRequest();
    const { errors } = await validateRegistration(req);

    expect(errors.firstName).toBe("First name must not be empty");
  });
  it("returns error object with a lastName error if the lastName is not in the request body", async () => {
    const req = mockRequest();
    const { errors } = await validateRegistration(req);

    expect(errors.lastName).toBe("Last name must not be empty");
  });
  ...
  it("returns result object with only firstName, lastName, email, and password if validation is successful", async () => {
    const req = mockRequest(validRequestBody);
    const { result } = await validateRegistration(req);

    expect(result.firstName).toBe(validRequestBody.firstName);
    expect(result.lastName).toBe(validRequestBody.lastName);
